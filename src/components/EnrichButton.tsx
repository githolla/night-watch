...
type Result = { configured: boolean; checked: number; emails: number; linkedins: number; error?: string };
...
export function EnrichButton({ accountId }: { accountId: string }) {
  ...
  async function run() {
    ...
    try {
      ...
      setMessage(`Checked ${json.checked} — ${json.emails} verified ${json.emails === 1 ? "email" : "emails"} and ${json.linkedins} LinkedIn ${json.linkedins === 1 ? "URL" : "URLs"} added.`);
      router.refresh();
    } catch (caught) {
