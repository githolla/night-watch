...
export function VersionAnalytics({ summary, error, source, days }: { summary: Summary; error?: string; source: string; days: string }) {
  ...
  return <section id="saved-versions" className="signal-analytics">
    ...
    {error ? <p role="alert">Version analytics could not load: {error}</p> : <>
      ...
      <div className="table-wrap"><table className="data-table"><thead><tr><th>Version</th><th>Gmail sends</th><th>Marked sent</th><th>Open detected</th><th>Gift views</th><th>Replies</th><th>Reply rate</th><th>Positive / referral</th><th>OOO</th></tr></thead><tbody>
        {summary.rows.map(r=><tr key={r.label}><td>{r.label}</td><td>{r.gmail}</td><td>{r.manual}</td><td>{r.gmail ? `${r.opens} / ${r.gmail} (${rate(r.opens,r.gmail)})` : 'Not tracked'}</td><td>{r.giftViews}</td><td>{r.replies}</td><td>{rate(r.replies,r.sent)}</td><td>{r.positive}</td><td>{r.ooo}</td></tr>)}
