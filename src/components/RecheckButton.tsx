...
export function RecheckButton({ accountId, name }: { accountId: string; name: string }) {
  ...
  const [phase, setPhase] = useState<"idle" | "sweep" | "research">("idle");
  ...
  return <span className="recheck">
    <button type="button" className="scan-stop" disabled={phase !== "idle"} onClick={recheck} title={`Read ${name}'s careers page, job boards and AI posts again, then put the research model on it`}>
      {phase === "sweep" ? "Reading careers page and posts…" : phase === "research" ? "Researching…" : "Check again now"}
    </button>
