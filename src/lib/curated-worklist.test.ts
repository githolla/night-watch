...
test("the active list is five revenue-qualified distinct operating businesses with researched buyers", () => {
  assert.equal(curatedDrafts.length, 5);
  assert.equal(new Set(curatedDomains).size, 5);
  ...
  assert.equal(new Set(curatedDrafts.map(row => row.subject)).size, 5);
  assert.equal(new Set(curatedDrafts.map(row => row.message)).size, 5);
});
