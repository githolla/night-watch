...
export async function analyzeCompany(input: AnalysisInput, options: { model: string; searches: number }, recordUsage?: UsageRecorder): Promise<CompanyAnalysis> {
  ...
  const [happening, hiring, peopleAndContacts, voices] = await Promise.all([
    ...
    settle("hiring", runSearchAgent(
      `${facts(input)}\n\n${POSITIONING}\n\nOpen roles already on file at ${input.name}:\n${rolesText}\n\nSearch LinkedIn Jobs, Indeed, Glassdoor, the company careers page and ZipRecruiter for ${input.name}'s current openings in operations, data and reporting, business systems and integration, RevOps, CRM administration, finance operations, and process or workflow automation. Only count operational roles Nine-67 could deliver as a built system; do NOT count roles that build AI as a product or do AI/ML research or engineering (machine learning or AI engineers, data scientists, research or applied scientists, computer vision, NLP, robotics) — those are not work Nine-67 does for a company. Read the postings you find. Then explain, as a sales strategist, what operational problem the company is trying to solve by hiring, for each such role, the specific product or automation Nine-67 would build to do that role's work instead of the company hiring a person — what it is (an internal tool, a data or reporting pipeline, an AI assistant, a self-running workflow) and what it would do — so build_instead reads as concrete builds, one per role, not a generic pitch, and a rough annual budget the hires represent. Return JSON only: {"read":"","build_instead":[""],"budget_estimate":null,"roles":[{"title":"","url":null,"posted_at":null,"why":""}]}. Only list roles you actually saw.`,
    ...
    settle("voices", runSearchAgent(
      `${facts(input)}\n\nPeople known to work there (search their names with site:linkedin.com/posts as well):\n${peopleText}\n\nFind what people who work at ${input.name} have said publicly in the last 180 days about AI, automation, data, systems, operations, efficiency, hiring difficulty, or problems in their own work: LinkedIn posts (site:linkedin.com/posts "${input.name}"), LinkedIn articles (site:linkedin.com/pulse "${input.name}"), podcast and webinar appearances, conference talks, interviews where they are quoted, and X posts. Only count a named person who works at ${input.name}; a company press release is not a voice. For each, give the author, their title, a verbatim quote of up to 400 characters, the URL, the exact date it was posted as YYYY-MM-DD (do not guess — leave null if the page shows no date), the platform, and a three-to-six-word topic. Prefer the most recent; anything older than 180 days is not a live voice. Up to 12. Return JSON only: {"voices":[{"author":"","title":"","quote":"","url":"","date":null,"platform":"","topic":""}]}.`,
