...
import { DEFAULT_RESEARCH_MODEL, fallbackChain, fallbackModelFor, isSupersededModel, webSearchToolType } from "./models.ts";
...
test("a rejected model falls back through current Sonnet models the key can use", () => {
  const chain = fallbackChain("claude-haiku-4-5");
  assert.ok(chain.includes("claude-sonnet-5"));
  assert.ok(!chain.includes("claude-haiku-4-5"));
  assert.ok(!fallbackChain("claude-sonnet-5").includes("claude-sonnet-5"));
});
