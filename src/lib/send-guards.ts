...
export function validateEmail(body:string,emailStatus:string,dailyCount:number,cap:number=baseDailyCap()){const urls=body.match(/https?:\/\/\S+/g)??[];const errors:string[]=[];if(emailStatus!=="verified")errors.push("Recipient email is not verified");if(dailyCount>=cap)errors.push(`Daily sender cap of ${cap} reached`);if(urls.length>1)errors.push("Email may contain at most one link");if(/<\/?[a-z][^>]*>|data:image|tracking pixel/i.test(body))errors.push("Email must be plain text without images or tracking");return errors}
