...
Choose one concrete business decision or handoff, or a relevant delivered example the recipient could inspect. State any unverified pain as a hypothesis, not inside knowledge. Connect it to a relevant delivered example or a clearly proposed first application. A process paragraph alone is not a reason to reply. Explain what the recipient would get from answering: a specific example, a useful comparison, or a scoped next step. Do not invent an attachment or an already-prepared personalized analysis.
Use around 90-115 words, at most 120, plus the saved greeting. End with the CTA question; the app adds the sender name. Name the company naturally. Use plain short subjects (3-7 words where possible), not a job title list or a slogan. Cover the required substance naturally. A verified client result may lead when it is stronger than a hypothetical problem. Do not force every email into the same paragraph order. Use the 20-app engagement only when scale is relevant, not by quota. These are cold emails to people who know nothing about Nine-67. Always explain that Nine-67 is an AI-first company with forward-deployed engineers who understand business and operations, build custom software with users, and stay through adoption. A sender signature is not a company introduction. Preserve any explicitly supplied personal introduction and greeting. Never add a sign-off pleasantry.
...
function qualityContext(domain?: string, senderName?: string): OutreachQualityContext {
  ...
  return { requireIntroduction: true, requireAIPositioning: true, reframe: brief?.pain_hypothesis.reframe, avoid: brief?.email_guidance.avoid, subjectIdeas: brief?.email_guidance.subject_ideas, cta: brief?.email_guidance.touch_1_cta, senderName };
}
