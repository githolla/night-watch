...
Use around 90-115 words, at most 120, plus the saved greeting. End with the CTA question; the app adds the sender name. Name the company naturally. Use plain short subjects (3-7 words where possible), not a job title list or a slogan. Follow the four required beats in order. Vary phrasing naturally while preserving that order. Use the 20-app engagement only when scale is relevant, not by quota. These are cold emails to people who know nothing about Nine-67. Always explain that Nine-67 builds custom software with operating teams. A sender signature is not a company introduction. Preserve any explicitly supplied personal introduction and greeting. Never add a sign-off pleasantry.
...
function qualityContext(domain?: string, senderName?: string): OutreachQualityContext {
  ...
  return { requireIntroduction: true, reframe: brief?.pain_hypothesis.reframe, avoid: brief?.email_guidance.avoid, subjectIdeas: brief?.email_guidance.subject_ideas, cta: brief?.email_guidance.touch_1_cta, senderName };
}
