...
const signal=z.object({type:z.enum(["job_post","job_cluster","exec_post","new_leader","funding","event","stack_change","other"]),summary:z.string(),source_url:z.url(),observed_at:z.string(),people:z.array(z.object({name:z.string(),title:z.string(),role_in_signal:z.string()})).default([]),post:z.object({text:z.string(),author_name:z.string(),author_title:z.string(),published_at:z.string(),reactions:z.number().int().nonnegative().nullable().default(null),comments:z.number().int().nonnegative().nullable().default(null),reposts:z.number().int().nonnegative().nullable().default(null),hashtags:z.array(z.string()).default([]),is_excerpt:z.boolean().default(false)}).optional(),source:z.object({headline:z.string(),publisher:z.string(),author_name:z.string().nullable().default(null),published_at:z.string(),excerpt:z.string()}).optional(),job:z.object({title:z.string(),department:z.string(),days_open:z.number(),reposted:z.boolean(),salary_max:z.number(),tools_named:z.array(z.string()),responsibilities:z.array(z.string())}).optional(),confidence:z.number().min(0).max(1)});
...
export async function scout(account:{name:string;domain:string;vertical?:string|null;employee_range?:string|null;careers_url?:string|null;news_query?:string|null;researchContext?:{aiSignal:string;sourceUrl:string;ceo:string;buyerTitles:string[];revenueBand:string;subSegment:string}}){const context=account.researchContext;const prompt=`Research ${account.name} (${account.domain}), a ${account.vertical??"target"} company with ${account.employee_range??"unknown"} employees.

Find 1-3 verifiable public developments that create a credible reason for an operations, automation, data, or AI conversation. FIRST look for a public LinkedIn post, article, interview, or conference comment from a named executive in the likely buyer group. Then check company news, ${account.careers_url??"the careers page"}, leadership changes, hiring clusters, acquisitions, funding, and technology changes. Prioritize the last 30 days. If nothing qualifies in 30 days, use the strongest relevant development from the last 180 days.
...
Return JSON only as {"signals":[{"type":"exec_post","summary":"why this matters","source_url":"https://...","observed_at":"YYYY-MM-DD","people":[{"name":"Full name","title":"Exact title","role_in_signal":"Author and operating owner"}],"post":{"text":"actual visible post text","author_name":"Full name","author_title":"Exact title","published_at":"ISO date or date","reactions":null,"comments":null,"reposts":null,"hashtags":[],"is_excerpt":true},"confidence":0.0}]}. Return an empty array only when no dated, source-backed development exists within 180 days.`;const response=await client().messages.create({model:process.env.ANTHROPIC_MODEL??"claude-sonnet-4-5",max_tokens:3500,messages:[{role:"user",content:prompt}],tools:[{type:"web_search_20250305",name:"web_search",max_uses:10} as unknown as Anthropic.Messages.Tool]});return scoutOutput.parse(jsonFrom(text(response.content))).signals.filter(s=>s.confidence>=.6)}
// @ts-expect-error documented server-side web search tool
export async function findPerson(account:string,signal:ScoutSignal){const response=await client().messages.create({model:process.env.ANTHROPIC_MODEL??"claude-sonnet-4-5",max_tokens:900,messages:[{role:"user",content:`Using public web search only, identify the most likely budget owner for this signal at ${account}: ${signal.summary}. Prefer a person directly named in the source or a publicly verified executive who owns the affected function. Return JSON only: {"name":"","title":"","linkedin_url":null,"alternates":[{"title":""}]}. Do not guess a name without public evidence.`}],tools:[{type:"web_search_20250305",name:"web_search",max_uses:4} as Anthropic.Messages.Tool]});return z.object({name:z.string(),title:z.string(),linkedin_url:z.string().nullable(),alternates:z.array(z.object({title:z.string()}))}).parse(jsonFrom(text(response.content)))}
export async function writeAngle(input:unknown){const response=await client().messages.create({model:process.env.ANTHROPIC_MODEL??"claude-sonnet-4-5",max_tokens:1400,messages:[{role:"user",content:`Draft outreach from this source-backed dossier: ${JSON.stringify(input)}. Ground every line in the supplied post or source. Name the specific public claim or operating change naturally; never invent familiarity, results, budget, or intent. Produce both LinkedIn and email copy even when contact details are unavailable. LinkedIn comment: two useful sentences replying to the actual post, with no pitch; leave empty only when the source is not a post. LinkedIn connection note: under 200 characters, specific to the source, no link. Email subject: under 6 words, lowercase. Email body: under 80 words, plain text, one low-friction question, no meeting request, one link maximum. For job signals, offer a free one-page JD teardown. Channel: intro for path 10; linkedin_only without verified email; linkedin_first for LinkedIn signals; otherwise email_first. Return JSON only: {"brief":"","why_now":"","channel":"email_first","linkedin_comment":"","linkedin_note":"","email_subject":"","email_body":""}.`}],});return angle.parse(jsonFrom(text(response.content)))}
