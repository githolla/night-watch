...
- Body: 50 to 90 words — short beats long for a cold first touch. Greet by first name. The first sentence names the specific thing seen — quote a short phrase of their post when there is one, otherwise the exact role they are hiring for or the development — and ties it to what THIS person owns. Then, in plain language, connect that work to one relevant approved case-study example or delivery detail. Use the 20-application engagement only occasionally; never put it in every email. Keep the distinction between deployed and built/ready examples.
