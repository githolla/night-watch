...
Find 1-3 verifiable public developments that create a credible reason for an operations, automation, data, or AI conversation. Prioritize the last 30 days. If nothing qualifies in 30 days, use the strongest relevant development from the last 180 days. Check company news, ${account.careers_url??"the careers page"}, public LinkedIn pages via search only, leadership changes, hiring clusters, acquisitions, funding, technology changes, conferences, and executive commentary.
...
Verify every returned claim with a public URL and an exact observed or publication date. Never turn the supplied clue into a fact unless the web source supports it. Allowed types: job_post, job_cluster, exec_post, new_leader, funding, event, stack_change, other. Never invent a URL, person, or date. Return JSON only as {"signals":[{"type":"...","summary":"...","source_url":"https://...","observed_at":"YYYY-MM-DD","people":[{"name":"","title":"","role_in_signal":""}],"job":{"title":"","department":"","days_open":0,"reposted":false,"salary_max":0,"tools_named":[],"responsibilities":[]},"confidence":0.0}]}. Return an empty array only when no dated, source-backed development exists within 180 days.`;const response=await client().messages.create({model:process.env.ANTHROPIC_MODEL??"claude-sonnet-4-5",max_tokens:3000,messages:[{role:"user",content:prompt}],tools:[{type:"web_search_20250305",name:"web_search",max_uses:8} as unknown as Anthropic.Messages.Tool]});return scoutOutput.parse(jsonFrom(text(response.content))).signals.filter(s=>s.confidence>=.6)}
