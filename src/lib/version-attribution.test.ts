...
test('identifies exact saved copy across sender and signature changes',()=>{
 ...
 assert.equal(result.label,'Gift'); assert.equal(result.edited,false);
});
test('manual edits preserve selected lineage, but cannot borrow another contact attribution',()=>{
 ...
 assert.equal(versionLabel(result),'Gift · edited'); assert.equal(result.revision,selectedMeta.revision);
...
test('one conversation, duplicate touches and followups produce one send and reply',()=>{
 ...
 assert.deepEqual(summary.rows[0],{label:'Gift',sent:1,gmail:1,manual:0,opens:1,giftViews:0,replies:1,positive:1,ooo:0});
...
test('source/date filters and edited rows have their own denominators',()=>{
 ...
 assert.equal(aggregateVersions([touch(),manual],{source:'manual'}).rows[0].label,'Gift · edited');
...
test('an unchanged archived selection stays unedited even after the draft bank changes',()=>{
 ...
 assert.equal(result.label,'Gift'); assert.equal(result.edited,false); assert.equal(result.revision,selectedMeta.revision);
});
...
test('self-test metadata is readable but is never an outreach cohort',()=>{
 ...
 assert.equal(testMeta.label,'Gift');
