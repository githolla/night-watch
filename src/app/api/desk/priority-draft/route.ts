...
/** Materialize one selected email on request. Never invent an address or send mail. */
export async function POST(request: Request) {
  try {
    const me = await requireUser();
    const input = await request.json();
    const selected = curatedDrafts.find(row => domainKey(row.domain) === domainKey(String(input.domain ?? "")));
