...
const drafts = research.map((row, id) => ({ ...row, id, recipientName: row.buyer.name, buyerSourceUrl: row.buyer.sourceUrl, tier: row.fit.split(":")[0], sourceUrl: row.trigger.sourceUrl, signal: row.trigger.fact, rationale: `${row.hypothesis} Limitations: ${Array.isArray(row.limitations) ? row.limitations.join(" ") : row.limitations}` }));
