...
type Posting = { id: string; title: string; url: string; location: string | null; department: string | null; family: string | null; source: string | null; posted_at: string | null; first_seen_at: string; active: boolean; description: string | null };
...
type CardRow = { id: string; person_id: string; status: string; score: number; channel: string; why_now: string; brief: string; email_subject: string | null; email_body: string | null; linkedin_note: string | null; linkedin_comment: string | null; linkedin_message: string | null; assigned_to: string; created_at: string; people: { full_name: string; title: string } | null };
...
const RUN_LABEL: Record<string, string> = { scheduled: "Nightly research", manual: "Research", sweep: "Hourly sweep", sweep_manual: "Sweep", analysis: "Nightly analysis", analysis_manual: "Agent swarm" };
const SOURCE_LABEL: Record<string, string> = { careers: "careers page", sitemap: "sitemap", jsonld: "careers page", web_search: "job boards", analysis: "job boards (agent)" };
const CHANNEL_LABEL: Record<string, string> = { email_first: "Email first", linkedin_first: "LinkedIn first, then email", linkedin_only: "LinkedIn only (no address on file)", intro: "Warm introduction" };
...
export default async function AccountPage({ params }: { params: Promise<{ domain: string }> }) {
  ...
  const [postings, posts, people, signals, cards, history, ownerRows] = live ? await Promise.all([
    db.from("job_postings").select("id,title,url,location,department,family,source,posted_at,first_seen_at,active,description").eq("account_id", live.id).order("active", { ascending: false }).order("family", { ascending: true, nullsFirst: false }).order("first_seen_at", { ascending: false }).limit(80).then((result) => (result.data ?? []) as Posting[]),
    ...
    db.from("cards").select("id,person_id,status,score,channel,why_now,brief,email_subject,email_body,linkedin_note,linkedin_comment,linkedin_message,assigned_to,created_at,people(full_name,title)").eq("account_id", live.id).order("score", { ascending: false }).limit(20).then((result) => (result.data ?? []) as unknown as CardRow[]),
  ...
  const bestDraft = openCards[0] ?? null;
  const bestDraftPerson = bestDraft ? people.find((person) => person.id === bestDraft.person_id) ?? null : null;
  const draftNote = analysis?.draft ?? null;
          ...
          <section className="card" id="who">
            ...
            {people.length ? <div className="table-wrap"><table className="data-table people-table"><thead><tr><th>Person</th><th>Email</th><th>Phone · LinkedIn</th><th>Found by</th><th>Draft</th></tr></thead><tbody>
              {ranked.map(({ person, cards: own, posts: theirPosts, touches: theirTouches }, index) => {
                ...
                const emailState = person.email ? (person.email_status === "verified" ? "verified" : person.email_status === "catch_all" ? "catch-all domain" : person.email_source === "pattern" ? "built from the company format, unverified" : person.email_source === "web" ? "seen on a public page, unverified" : person.email_source === "hunter" ? "found by Hunter, unverified" : "unverified") : "";
                return <tr key={person.id} className={index === 0 ? "is-first" : ""}>
                  ...
                  <td className="cell-who">{person.phone ? <span>{person.phone}</span> : null}<small>{person.linkedin_url ? <a href={person.linkedin_url} target="_blank" rel="noreferrer">LinkedIn profile ↗</a> : "no LinkedIn URL"}{person.contact_notes ? <> · {person.contact_notes.length > 90 ? `${person.contact_notes.slice(0, 90)}…` : person.contact_notes}</> : null}</small></td>
            ...
            : <p className="card-empty">{outreach ? "Nobody found yet. Press Research this company now: the agents look on LinkedIn, the company site and press, then look up contact details." : "Held companies are not enriched."}{!process.env.APOLLO_API_KEY && !process.env.HUNTER_API_KEY ? " Verified addresses need APOLLO_API_KEY or HUNTER_API_KEY in the deploy." : ""}</p>}
          </section>
          ...
          <section className="card pad" id="drafts">
            ...
            {bestDraft ? <div className="draft">
              ...
              <div className="draft-channels">
                ...
                {bestDraft.linkedin_note && <div className="draft-channel"><header><span>LinkedIn · connection note</span><CopyButton text={bestDraft.linkedin_note} label="Copy" /></header><p>{bestDraft.linkedin_note}</p></div>}
                {bestDraft.linkedin_message && <div className="draft-channel"><header><span>LinkedIn · message</span><CopyButton text={bestDraft.linkedin_message} label="Copy" /></header><pre className="who-body">{bestDraft.linkedin_message}</pre></div>}
                {bestDraft.email_body && <div className="draft-channel"><header><span>Email{bestDraft.email_subject ? ` · ${bestDraft.email_subject}` : ""}</span><CopyButton text={`Subject: ${bestDraft.email_subject ?? ""}\n\n${bestDraft.email_body}`} label="Copy" /></header><pre className="who-body">{bestDraft.email_body}</pre></div>}
              </div>
              {openCards.length > 1 && <p className="muted">{openCards.length - 1} more open {openCards.length === 2 ? "draft" : "drafts"} for this company on the desk.</p>}
            </div> : <div className="draft-none"><span>No draft yet</span><p>{draftNote ? draftNote.reason : analysis ? "The analysis ran before drafts were written from it. Press Research this company now." : outreach ? "The agent swarm has not run on this company yet. Press Research this company now: it finds the people, their contact details, the roles and posts, then writes the draft." : "Held companies do not get drafts. Put the company on the reach-out list first."}</p></div>}
          </section>

          <EvidenceTabs tabs={[
            { id: "roles", label: "Roles to build instead", count: targetPostings.length, content: <div>
              {analysis?.hiring.read && <div className="happening-block" style={{ paddingTop: 16, paddingBottom: 8 }}><span className="eyebrow">What the hiring means{analysis.hiring.budgetEstimate ? ` · about ${analysis.hiring.budgetEstimate} a year in hires` : ""}</span><p className="brief-text">{analysis.hiring.read}</p>{analysis.hiring.buildInstead.length > 0 && <><span className="eyebrow">What Nine-67 would build instead</span><ul className="happening-build">{analysis.hiring.buildInstead.map((line, index) => <li key={index}>{line}</li>)}</ul></>}</div>}
              {targetPostings.length ? <ul className="click-list">{targetPostings.map((posting) => <li key={posting.id}><a href={posting.url} target="_blank" rel="noreferrer"><b className="click-tag">{FAMILY_LABEL[posting.family as JobFamily] ?? posting.family}</b><div><strong>{posting.title}</strong>{posting.description && posting.source === "analysis" && <p className="role-why">{posting.description}</p>}<small>{[posting.department, posting.location, posting.posted_at ? `posted ${date(posting.posted_at)}` : `seen ${date(posting.first_seen_at)}`, SOURCE_LABEL[posting.source ?? ""] ?? host(posting.url)].filter(Boolean).join(" · ")}</small></div><em>Open ↗</em></a></li>)}{otherPostings.length > 0 && <li><details className="account-more"><summary>{otherPostings.length} other postings read (not a target family, or closed)</summary><ul>{otherPostings.slice(0, 40).map((posting) => <li key={posting.id}><a href={posting.url} target="_blank" rel="noreferrer">{posting.title}</a>{!posting.active && <small> · closed</small>}</li>)}</ul></details></li>}</ul> : <p className="card-empty">{live.careers_status === "none" ? "No careers page could be found, and the job-board search found nothing in a target family." : live.careers_status ? "Careers page read; nothing in a target family is open." : "Careers page not read yet."}</p>}
            </div> },
