...
LinkedIn is where the posts and the people are, and it cannot be crawled or automated (see Guardrails). What can be done is to search for it: with `SERPER_API_KEY` (Google results) or `BRAVE_API_KEY` set, every sweep runs up to `SWEEP_SEARCH_QUERIES` (40) plain searches per company on the extensive pass: LinkedIn profiles by the buyer titles and by department (operations, IT, data, finance, revenue, customer, transformation, leadership), LinkedIn posts from six angles (AI, data and systems, hiring, efficiency, announcements), articles, posts by every person already on file, and what is said on X, Medium, Substack, YouTube and podcasts, and reads the results by URL pattern with no model (`src/lib/linkedin-discovery.ts`). A profile result gives a person, title and profile URL; a post result gives the author and the first lines, stored under Posts with platform `linkedin`, and a post about AI or automation becomes a signal. This is the cheapest and most reliable source of both people and posts; without a search key, only the model's own web search is used.
