...
The desk has two one-time buttons for filling an empty database. "Initial populate: sweep all now" reads every careers page immediately, ignoring the daily cooldown. "Initial populate: research N companies, 5 searches each" researches up to `POPULATE_ACCOUNT_LIMIT` companies (default 300), hiring companies first, ignoring the 7-day cooldown, with `POPULATE_MAX_SEARCHES` web searches each (default 5) until `POPULATE_RUN_BUDGET_USD` (default $25) of measured spend; anything past the budget stays queued and the next press continues it. Both show the same live run log as the nightly runs.
