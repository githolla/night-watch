...
- Research, analysis and writing use the current Sonnet (`claude-sonnet-5`) by default and the cheap search agents use Haiku 4.5; `src/lib/models.ts` holds the defaults and picks the web-search tool version each model takes. A configured model the API no longer knows is retried once on `ANTHROPIC_FALLBACK_MODEL` (default: the current Sonnet) with a warning in the logs, so a retired model id does not fail every company.
