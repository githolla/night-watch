...
After replacing the workbook, run `python3 scripts/export-target-cut.py` (needs `openpyxl`) to refresh `data/Nine67_Outbound_Targets_Cut.csv`, then `npm run targets:generate`, then sync on the Accounts page. Migration `supabase/migrations/0009_outreach_tiers.sql` adds the tier and reach-out columns; `0012_analysis_drafts.sql` adds the LinkedIn message on drafts and the `analysis` posting source. The app shows the SQL to paste whenever the database is behind the code.
...
## What one company gets, and where it shows

Press **Research this company now** on a company page (or let the scan run) and three passes happen in order: the sweep reads the careers page, job boards and public LinkedIn results; the research pass looks for one dated signal; then the agent swarm runs. The swarm is what fills the page: people and their contact details (People and contact details), the roles it read on LinkedIn Jobs, Indeed and the careers site with what Nine-67 would build instead (Roles to build instead), their public posts and quotes (AI posts), what is happening at the company, the brief (Why now), and **the draft**: a LinkedIn reply, a connection note, a LinkedIn message and an email for the person the brief says to write first. The draft is written whenever the brief rates the fit at `DRAFT_FIT_FLOOR` (40) or more, whether or not any single signal clears the card threshold; it goes to the desk like any other dossier and the company page shows it under the brief with a copy button per channel. A draft someone has already approved, edited, sent or dismissed is never rewritten by a later analysis. If no draft was written, the page says why (fit under the floor, nobody on file, held company).

## Working email addresses

Night Watch finds people without any paid data source, and builds an address for each from the company's format once it has seen one real address; built addresses are marked unverified and the send guard keeps them out of automatic sends. To get addresses that are known to work, set one of two keys. `HUNTER_API_KEY` is the cheaper route: each built or unverified address is checked against Hunter's verifier (valid, catch-all or invalid; a wrong guess is cleared), and anyone without an address is looked up with Hunter's finder. `APOLLO_API_KEY` adds Apollo's people match and title search, which return an address only when Apollo has verified it. With the key set, a company page shows **Enrich contacts with Apollo**, which fills a verified email and LinkedIn URL for the people already on file on demand, so existing contacts are enriched without waiting for the next contacts sweep. When no address and no example are found anywhere, the format falls back to a best-guess `first.last@` marked as a guess (never used for an automatic send), so the contact column is never empty. Clay is not needed: it is a waterfall over providers like these two, useful at volume when one provider's hit rate is not enough, and it would need its own integration.

## Cost controls

- Everything runs on the cheapest current model, Haiku 4.5, by default: research, the analysis swarm, the writing and the search agents. `src/lib/models.ts` holds the defaults and picks the web-search tool version each model takes. Raise a single role above Haiku only where the quality pays for itself; the writing model is one call per company, so `ANTHROPIC_WRITING_MODEL=claude-sonnet-5` buys sharper drafts for a few cents each. A configured model the API no longer knows is retried once on `ANTHROPIC_FALLBACK_MODEL` with a warning in the logs, so a retired model id does not fail every company.
- The nightly analysis is change-driven, not a fixed wheel. The one-time baseline (the desk's "Initial populate" / first scan) runs the swarm on every reach-out company that has any signal. After that, a scheduled or manual analysis spends the swarm only on companies where a role, post or person entered the database since the last analysis, and never on a company with no signal at all. So a quiet night costs almost nothing; the cost tracks how much genuinely changed. `ANALYSIS_COOLDOWN_DAYS` (14) is the floor between re-analyses of the same company. Fresh intent the sweep finds between analyses still drafts a card on its own, cheaply.
...
After the sweep and the research pass, each reach-out company gets a deep analysis by four search agents working at once and a synthesizer (`src/lib/analysis.ts`): what is happening at the company in the last six months (deals, leaders, initiatives, systems, stated problems), what the open hiring means and which of it Nine-67 would build instead with a rough budget, the buying map (decision owners and hiring managers with LinkedIn, tenure and why they matter, plus any addresses seen so the email format can be learned), what people there have said publicly (verbatim quotes with URLs), and a contact-details agent that runs once the people are known and finds published work emails, direct or mobile numbers, LinkedIn URLs, the company's main line and address, only from pages where they appear, never constructed. The synthesizer writes the manager's brief from only that evidence: why now, the angle, who to write to first and why, a personalised opener, likely pushback with answers, the next step, and a fit score. People, quotes and qualifying signals are stored like everything else, so drafts get written from them. The analysis is stored on the account (`analysis`, `analysis_at`) and shown at the top of the company page. `ANALYSIS_MODEL` (the research model), `ANALYSIS_SEARCHES` per agent (6), `ANALYSIS_RUN_BUDGET_USD` per press (100), `ANALYSIS_COOLDOWN_DAYS` (14), `ANALYSIS_CONCURRENCY` (2). The scan runs it after research, hottest companies first; the nightly cron (`/api/cron/analysis`) re-analyses up to 40 companies past their cooldown; "Research this company now" on a company page runs all three passes for that company. The roles the hiring agent reads on job boards are stored as postings (source `analysis`) so the Roles tab, the intelligence score and the hiring signal all see them, and the draft is written from the brief (see above).

## LinkedIn, from public search results

LinkedIn is where the posts and the people are, and it cannot be crawled or automated (see Guardrails). What can be done is to search for it. Every sweep runs up to `SWEEP_SEARCH_QUERIES` (40) searches per company on the extensive pass, carried out by search agents (the search model with its web-search tool, four queries per agent so the verbatim answer never overflows, agents in parallel, `src/lib/web-search.ts`), or by Serper or Brave if a key happens to be set: LinkedIn profiles by the buyer titles and by department (operations, IT, data, finance, revenue, customer, transformation, leadership), LinkedIn posts from six angles (AI, data and systems, hiring, efficiency, announcements), articles, posts by every person already on file, and what is said on X, Medium, Substack, YouTube and podcasts, and reads the results by URL pattern with no model (`src/lib/linkedin-discovery.ts`). A profile result gives a person, title and profile URL; a post result gives the author and the first lines, stored under Posts with platform `linkedin`, and a post about AI or automation becomes a signal. The results are read by rule, so the agents only fetch; they never judge.

## People and addresses

Contacts come from every source there is, not only Apollo: the company's own leadership and team pages (read directly, no model), a web search over LinkedIn profile results, press releases and bios (`ANTHROPIC_SEARCH_MODEL`), Apollo title search when `APOLLO_API_KEY` is set, and the CEO from the target file. The managers who own the budget for each open role are looked for first, then the file's likely buyers, then the CEO; plus LinkedIn profiles from public search results and an Apollo leadership search by seniority (owners, C-suite, VPs, heads, directors); up to `SWEEP_CONTACTS_PER_COMPANY` (default 12) per company, 40 on the extensive pass. The company's address format is then learned from any address already on file, or from addresses seen on public pages, stored on the account (`email_pattern`, `pattern_confidence`), and used to build an address for everyone there without one. Built addresses are marked unverified and shown as such on the company page; the send guard keeps them out of automatic sends, so they are for a person to verify and use by hand.
